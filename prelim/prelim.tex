	...
	\begin{frame}
		...
		\begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
			...
			\begin{itemize}
				...
				\item Neutrinos only exist as left handed particles, so via the Higgs Mechanism and the Yuakawa interaction term, the Standard model predicts they are \footnotesize{\textbf{massless}}.
			\end{itemize}
...
\begin{frame}
		...
		\begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
			...
			\begin{itemize}
				\setlength\itemsep{2em}
				\item Proposed $0\nu\beta\beta$ search using bolometric array of 1596 Li$_2$MoO$_4$ crystals, to be deployed in the CU
			\end{itemize}
