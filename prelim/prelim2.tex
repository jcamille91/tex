...
\section{Motivation and Background}
	...
	\begin{frame}
		...
		\begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
			...
			\column{.45\textwidth} % Left column and width
			%\textbf{Heading}	
			Majorana fermions can be seen as solutions to the Dirac equation with an extra constraint:
			$\psi = \psi^C$				
			...
			\bigg[\ \boldsymbol{\hat{b}(\overline{k},s)}u(\overline{k},s)e^{-ik^{\mu}x_{\mu}}
			 ...
			 Q = \int d^3k \  \left(b(k)b^{\dagger}(k) - b(k)b^{\dagger}(k)\right)
			\end{eqnarray*}
			}
			$\implies$ Majorana fermions are \textbf{chargeless}, and their \textbf{own anti-particles}
			\column{.5\textwidth} % Right column and width
...
%			(CUORE-0, GERDA, XENON, EXO, NEMO)
	...
	\begin{frame}
		...
		\begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
			...
			\column{.5\textwidth} % Right column and width
			\includegraphics[width=5.7cm]{~/research/figures/prelim/bbspectrum.png}
